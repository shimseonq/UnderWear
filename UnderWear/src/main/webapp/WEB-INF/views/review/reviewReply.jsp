...
$(function() {
	...
	//	$(".reset_btn").click();
		...
		pwdReset(this);							
		pwdView(currLi);					//li를 매개변수로 줘서 비밀번호를 보이게 함
		btnKind="delBtn";
	});
	
	/* 댓글 수정 위한  "수정완료" 버튼 클릭시 Ajax연동처리 */
	$(document).on("click", ".update_btn", function() {
		var rrp_no = $(this).parents("li").attr("data-num");
		var rrp_content = $("#content").val();
		if(!chkData("#content","댓글 내용을"))return;
		
		else{
			$.ajax({
				url:'/reviewReply/'+rrp_no+".do",
				type:"put",						//put : 수정하기
				headers:{"Content-Type":"application/json",
						"X-HTTP-Method-Override":"PUT"	
				},
				data:JSON.stringify({
					rrp_content:rrp_content
				}),
				dataType:"text",
				error:function(){
	...
	$(document).on("click",".pwdCheckBut", function(){
		...
		if(!formCheck(pwd, msg, "비밀번호를")) return;
			...
			$.ajax({
				url:"/reviewReply/replyPwdCheck.do",
				type:"post",
				data:"rrp_no="+replyNum+"&rrp_pwd="+pwd.val(),
				dataType:"text",
				error:function(){
					alert('시스템 오류입니다 관리자에게 문의하세요');
				},
