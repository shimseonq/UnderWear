         ...
         <c:choose>
            <c:when test="${not empty orderList}">
               <c:forEach var="orders" items="${orderList }" varStatus="status">
                  <tr class="tac" data-num="${orders.o_no}" data-delivery="${orders.o_delivery }">
                  	<td><input type="checkbox" name="item" class="item" value="${orders.o_no}"/></td>
                     <td>${orders.o_no}</td>
                     <td>${orders.c_id}</td>
                     <td>${orders.p_name}</td>
                     <td>${orders.pr_01}</td>
