...
public class FileUploadUtil {
	...
	public static String fileUpload(MultipartFile file, HttpServletRequest request, String fileName) throws IOException {
		...
		if (org_name != null && (!org_name.equals(""))) {
			real_name = fileName + "_" + System.currentTimeMillis() + "_" + org_name;	// 저장할 파일 이름. 중복 되지 않게 함.
			...
			File fileAdd = new File(docRoot + "/" + real_name);		// 물리적인 경로.
			logger.info("업로드할 파일(fileAdd) : " + fileAdd);
			
			// 파일 업로드 (저장)
			file.transferTo(fileAdd);		// 실질적인 작업을 함. (실제 저장을 함.)
		}
		return real_name;
	}
	...
	public static void fileDelete(String fileName, HttpServletRequest request) throws IOException {
		...
		String dirName = fileName.substring(0, fileName.indexOf("_"));	// 파일이름에서 _가 있는 첫번째를 찾아옴.
	...
	public static String makeThumbnail(String fileName, HttpServletRequest request) throws Exception {
		...
		// resize(대상[BufferedImage타입], 원본비율, 높이 또는 너비, 크기
		
		String thumbnailName = "thumbnail_" + fileName;		// 파일 객체 생성.
		...
		String formatName = fileName.substring(fileName.lastIndexOf(".") +1);	// 확장자를 찾아옴.
